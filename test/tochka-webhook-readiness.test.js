...
test('webhook readiness requires fresh operations and complete Tochka to DDS propagation', () => {
...
test('Tochka webhook checks propagation readiness before fetching or mirroring live balances', () => {
  ...
  assert.ok(readinessAt >= 0, 'webhook must read propagation readiness');
  assert.ok(fetchAt > readinessAt, 'readiness must be checked before live balance fetch');
  assert.ok(mirrorAt > readinessAt, 'readiness must be checked before mirror write');
  assert.match(webhookSource, /status\(409\).*Tochka operations not ready for balance mirror/s);
});
