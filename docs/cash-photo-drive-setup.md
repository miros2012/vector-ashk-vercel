...
Distribute the same shared link once through the internal staff instructions. Managers select their branch, choose a photo and press **Отправить журнал**. A new manager uses the same link; there is no account creation or per-employee link issue. This is a shared credential: individual holders cannot be revoked separately. If it leaks, rotate its random credential and verifier. The shared-link verifier is canonical in `lib/cash-photo-shared-key.js` and is not read from Vercel environment variables. Rotating or disabling the shared link therefore requires an explicit reviewed code change and deployment; environment imports cannot silently invalidate the existing link. Branch links remain independently managed.
