...
- статус `Оценка` является плановой строкой и не активирует `DEC-UNCONF-OBL`; неподтверждёнными считаются только открытые строки со статусом `Требует...`;
- `DEC-CRIT-DUE` должен сначала включать все просроченные открытые обязательства независимо от приоритета, а при отсутствии просрочки — ближайшие критические платежи в окне 0–3 дня;
- статус `Просрочено — частично оплачено` остаётся открытым до полного закрытия остатка.
...
Активные задачи создаются Decision Engine в листе `Решения` и выводятся на панели собственника. `Owner Action Queue` хранит только явно инициированные пользователем команды жизненного цикла и не должна автоматически переводить решение в работу или завершать его.
