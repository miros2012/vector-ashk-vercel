...
function validRecovery(result) {
  if (!result || typeof result !== 'object' || Array.isArray(result)) return false;
  const fields = ['attempted', 'recognized', 'stillPending', 'failed'];
  if (!fields.every(field => Number.isInteger(result[field]) && result[field] >= 0)) return false;
  return result.attempted <= 3
    && result.recognized + result.stillPending + result.failed === result.attempted;
}

async function recoverPending(services) {
  if (!services.retryService?.retryPendingConcurrent) {
    return { attempted: 0, recognized: 0, stillPending: 0, failed: 0 };
  }
  const result = await services.retryService.retryPendingConcurrent(3, {}, { reconcileDraft: false });
  if (!validRecovery(result)) throw new Error('pending recovery result invalid');
  return result;
}

async function execute(services, deploymentSha) {
  // This immutable, nonfinancial fixture is the entire caller-controlled capability.
  // After the fixture is verified, the server may additionally perform one fixed,
  // bounded recovery pass over its own pending-photo queue. Request data never
  // supplies an image, branch, filename, Drive id, archive row, limit or filter.
  ...
  const recovery = await recoverPending(services);
