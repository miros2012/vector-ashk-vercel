...
function cityRow(selectedRows, idx, reportDate, snapshot) {
...
function snapshotRows(rows, idx, reportDate, snapshot) {
  ...
  return [cityRow(selectedRows, idx, reportDate, snapshot), ...managers];
}
...
export function buildRopMorningDashboard({ controlValues, asOfDate } = {}) {
  ...
  const closedRows = snapshotRows(rows, idx, reportDate, 'ВЧЕРА — ЗАКРЫТО');
  const hasLive = rows.some(row => dateOnly(row[idx.get('Дата')]) === currentDate);
  const liveRows = hasLive ? snapshotRows(rows, idx, currentDate, 'СЕГОДНЯ — НА СЕЙЧАС') : [];
