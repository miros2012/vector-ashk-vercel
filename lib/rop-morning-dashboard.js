...
function forecastFromPace(monthPlan, planToDate, factMtd) {
  ...
  const elapsedShare = elapsedPlan / plan;
  return roundMoney(fact / elapsedShare);
}

const HEADERS = [
  'Дата отчёта','Уровень','Менеджер','Филиал','План месяца','План к дате',
  'Факт вчера','Факт с начала месяца','Выполнение','Дефицит к плану на дату',
  'Прогноз месяца','Отклонение прогноза','Контрольный план менеджера',
  'Личный факт вчера','Личный факт с начала месяца','Новых договоров',
...
function chooseReportDate(rows, dateIndex, asOfDate) {
...
function managerRow(row, idx, reportDate) {
  ...
  const existingNote = text(row[idx.get('Примечание')]);
...
function cityRow(selectedRows, idx, reportDate) {
  ...
  const newContracts = Math.round(selectedRows.reduce((total, row) => total + toNumber(row[idx.get('Новых договоров с начала месяца')]), 0));
  const fullPaid = Math.round(selectedRows.reduce((total, row) => total + toNumber(row[idx.get('100% оплаченных новых договоров')]), 0));
  const status = statusFromRatio(ratio);
...
export function buildRopMorningDashboard({ controlValues, asOfDate } = {}) {
  ...
  const reportDate = chooseReportDate(rows, idx.get('Дата'), asOfDate);
  const selectedRows = rows.filter(row => dateOnly(row[idx.get('Дата')]) === reportDate);
  if (!selectedRows.length) throw new Error('ROP control has no rows for report date');

  const managers = selectedRows
    .map(row => managerRow(row, idx, reportDate))
    .sort((a, b) => {
      const rank = { 'СЕГОДНЯ': 0, 'КОНТРОЛЬ': 1, 'ОК': 2 };
      const priorityDiff = (rank[a[18]] ?? 9) - (rank[b[18]] ?? 9);
      return priorityDiff || String(a[3]).localeCompare(String(b[3]), 'ru') || String(a[2]).localeCompare(String(b[2]), 'ru');
    });

  const city = cityRow(selectedRows, idx, reportDate);
  return {
    reportDate,
    values: [HEADERS, city, ...managers],
    metrics: {
      managers: managers.length,
      branches: new Set(selectedRows.map(row => text(row[idx.get('Филиал')])).filter(Boolean)).size,
      todayPriority: managers.filter(row => row[18] === 'СЕГОДНЯ').length
    }
  };
}
