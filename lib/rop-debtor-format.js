...
export function buildDebtorPriorityFormatRequests({ sheetId, rowCount = 500, existingConditionalRuleCount = 0 } = {}) {
  ...
  requests.push(
    ...
    conditionalRule(id, rows, '=AND($A2<>"",$S2="ОПЛАЧЕНО")', color(217, 234, 211), 0),
    conditionalRule(id, rows, '=AND($A2<>"",OR($S2="ОБЕЩАНИЕ",$S2="ЧАСТИЧНАЯ ОПЛАТА"))', color(255, 242, 204), 1),
    conditionalRule(id, rows, '=AND($A2<>"",OR($S2="",$S2="НЕ СВЯЗАЛИСЬ",$S2="ОТКАЗ/ЭСКАЛАЦИЯ"))', color(244, 204, 204), 2)
  );
