...
export function createSyncHoursHandler({
  ...
  return async function syncHoursHandler(req, res) {
    ...
    try {
      ...
      const errorClass = transient ? 'HOURS_TRANSPORT' : 'HOURS_SYNC_FAILED';
      console.error(JSON.stringify({event:'sync-hours-failure',errorClass,
        ...(Number.isInteger(status) && status >= 400 && status <= 599 ? {status} : {})}));
      return res.status(transient ? 503 : 500).json({ok:false,errorClass,error:'Hours sync failed'});
    }
  };
}
