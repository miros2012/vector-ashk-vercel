...
function criticalPaymentDue(snapshot, now) {
  ...
  const eligible = payments
    .map((payment) => ({
      ...
      amount: nullableFinite(payment?.amount),
    ...
    .filter((payment) => {
      ...
      const actionableAmount = payment.amount === null || payment.amount > 0;
      return actionableAmount && (days < 0 || (critical && days <= 3));
    });
  ...
  const amount = actionable.some((payment) => payment.amount === null)
    ? null
    : actionable.reduce((sum, payment) => sum + payment.amount, 0);
