...
export function createFinanceCycleStore({sheets,spreadsheetId,lease=createGoogleSheetsFinanceRunStore({sheets,spreadsheetId}),sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms))}) {
  const api=sheets.spreadsheets.values;
  const readRows=async()=> {
    for(let attempt=0;attempt<3;attempt++) {
      try { return (await api.get({spreadsheetId,range:"'__vercel_control'!A1:B1009",valueRenderOption:'UNFORMATTED_VALUE'})).data.values||[]; }
      catch(error) {
