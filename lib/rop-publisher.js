...
export async function syncRopSourceThenPublishTarget({ refreshSource, publishTarget } = {}) {
  ...
  let source;
  try {
    source = await refreshSource();
  } catch {
    return sanitizeFinanceStageFailure({ statusCode: 502, errorClass: 'ROP_BUILD' });
  }
  if (!source?.ok) return source;

  let target;
  try {
    target = await publishTarget();
  } catch {
    return sanitizeFinanceStageFailure({ statusCode: 502, errorClass: 'ROP_PUBLISH' });
  }
  if (!target?.ok) return sanitizeFinanceStageFailure({ statusCode: 502, errorClass: 'ROP_PUBLISH' });
