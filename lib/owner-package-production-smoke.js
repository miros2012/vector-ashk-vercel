import { runOwnerPackageVerifierStage } from './owner-package-verifier-stage.js';
...
function observationClock(value) {
  ...
  const fixed = normalizeTimestamp(value, 'now');
  return () => fixed;
}
...
export async function verifyOwnerPackageProduction({
  ...
  const observeNow = observationClock(now);
  ...
  const nowValue = observeNow();

  await runOwnerPackageVerifierStage('BODY_SHAPE', () => {
  ...
  await runOwnerPackageVerifierStage('CACHE_CONTROL', () => {
    if (!/(?:^|,)\s*no-store(?:\s*(?:,|$)|\s*=)/i.test(cacheControl)) {
      throw new Error('Cache-Control must include no-store');
    }
  });

  const body = await runOwnerPackageVerifierStage('BODY_SHAPE', async () => {
    let parsed;
    try {
    ...
    if (!parsed || typeof parsed !== 'object' || parsed.ok !== true || !parsed.package || typeof parsed.package !== 'object') {
  ...
  const { withdrawal } = await runOwnerPackageVerifierStage('SAFE_WITHDRAWAL', () => {
    const normalizedWithdrawal = normalizeFiniteNumber(
    ...
    if (normalizedWithdrawal !== summaryWithdrawal) {
      throw new Error('safeWithdrawal facts conflict');
    }
    if (expectedWithdrawal !== undefined && normalizedWithdrawal !== expectedWithdrawal) {
      throw new Error('safeWithdrawal does not match expectation');
    }
    return { withdrawal: normalizedWithdrawal };
  });

  const { policyBlockers, operatingReserve } = await runOwnerPackageVerifierStage('POLICY', () => {
    const normalizedBlockers = normalizePolicyBlockers(ownerPackage?.policy?.blockers);
    if (requiredBlocker && !normalizedBlockers.includes(requiredBlocker)) {
      throw new Error('required policy blocker is missing');
    }
    const normalizedReserve = normalizeOperatingReservePolicy(
      ownerPackage?.policy?.operatingReserve,
      normalizedBlockers
    );
    return {
      policyBlockers: normalizedBlockers,
      operatingReserve: normalizedReserve
    };
  });

  const businessDate = await runOwnerPackageVerifierStage('BUSINESS_DATE', () => {
    if (!ownerPackage.snapshot || typeof ownerPackage.snapshot !== 'object') {
      throw new Error('snapshot is required');
    }
    const normalized = normalizeBusinessDate(ownerPackage.snapshot.businessDate);
    if (normalized !== currentBusinessDate(nowValue.timestamp)) {
      throw new Error('snapshot businessDate does not match current business date');
    }
    return normalized;
  });

  const { generatedAt, ageMs } = await runOwnerPackageVerifierStage('FRESHNESS', () => {
    const normalizedGeneratedAt = normalizeTimestamp(ownerPackage.snapshot.generatedAt, 'snapshot generatedAt');
    const age = nowValue.timestamp - normalizedGeneratedAt.timestamp;
    if (age < 0) throw new Error('snapshot is from the future');
    if (age > freshnessLimit) throw new Error('snapshot is stale');
    return { generatedAt: normalizedGeneratedAt, ageMs: age };
  });
