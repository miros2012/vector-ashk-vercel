...
const HARD_REVIEW_REASON = /(нет даты|нет филиала|нет описания|заполните только расход или приход|нет остатка по журналу|уверенность ИИ ниже 85%|ИИ просит проверить распознавание|арифметика журнала не сходится|кошелёк не найден или неактивен)/i;
...
export function createCashJournalPipeline({sheets,spreadsheetId,now=()=>new Date()}={}) {
  ...
  async function reconcileDraftBacklog(limit = 100) {
    ...
    const [draftResp, rulesResp, walletResp, ddsCommentsResp] = await Promise.all([
      values.get({spreadsheetId,range:`'${DRAFT_SHEET}'!A2:N`,valueRenderOption:'UNFORMATTED_VALUE'}),
      values.get({spreadsheetId,range:`'${RULES_SHEET}'!A2:F`,valueRenderOption:'UNFORMATTED_VALUE'}),
      values.get({spreadsheetId,range:`'${WALLETS_SHEET}'!A2:E`,valueRenderOption:'UNFORMATTED_VALUE'}),
      values.get({spreadsheetId,range:`'${DDS_SHEET}'!M${DDS_FIRST_ROW}:M${DDS_LAST_ROW}`,valueRenderOption:'UNFORMATTED_VALUE'})
    ...
    const wallets = buildWallets(walletResp?.data?.values || []);
    const ddsCounts = new Map();
    for (const row of ddsCommentsResp?.data?.values || []) {
      const id = ddsCashId(row?.[0]);
      if (id) ddsCounts.set(id, (ddsCounts.get(id) || 0) + 1);
    }
    ...
    for (let index = 0; index < rows.length && summary.scanned < max; index += 1) {
      ...
      if (id && Number(ddsCounts.get(id) || 0) >= expectedParts) {
        await values.update({
          spreadsheetId,
          range:`'${DRAFT_SHEET}'!M${rowNumber}:N${rowNumber}`,
          valueInputOption:'RAW',
          requestBody:{values:[['Перенесено','[AUTO-RECHECK] ДДС уже содержит операцию; статус восстановлен']]}
        });
        const fp = fingerprint(row[1], row[2], row[4], row[5], row[6]);
        if (fp && !claimedFingerprints.has(fp)) claimedFingerprints.set(fp, rowNumber);
        summary.recovered += 1;
        continue;
      }
