...
function existingFingerprints(rows) {
  ...
  for (let i=0;i<(Array.isArray(rows)?rows.length:0);i++) {
    ...
    if(fp) map.set(fp,{row:i+2,id:text(row[0]),status:text(row[12]),data:[...row]});
  }
  return map;
}
...
const HARD_REVIEW_REASON = /(нет даты|нет филиала|нет описания|заполните только расход или приход|нет остатка по журналу|уверенность ИИ ниже 85%|арифметика журнала не сходится|кошелёк не найден или неактивен)/i;

function reclassifyExistingDraftRow(row, rules, wallets) {
  const date = ruDateSerial(row?.[1]);
  const branch = text(row?.[2]);
  const description = text(row?.[3]);
  const expense = Math.max(0, numberValue(row?.[4]));
  const income = Math.max(0, numberValue(row?.[5]));
  const balancePresent = text(row?.[6]) !== '';
  const errors = [];
  ...
  if (!balancePresent) errors.push('нет остатка по журналу');

  const rule = classify(description, rules);
  let type = rule.type;
  if (type !== 'Инкассация') {
  ...
  return { type, article, fromWallet, toWallet, errors, rule };
}
...
export function buildCashDraftRows({photo,data,rules,wallets,existingRows}={}) {
  ...
  const rows=[]; const meta=[]; const existingReady=[];
  ...
  operations.forEach((op,index)=>{
    ...
    if(fp && existing.has(fp)) {
      const matched = existing.get(fp);
      meta.push({index,duplicate:true,status:'Дубль — уже учтено',existing:matched});
      if (
        matched?.status === 'Готово к переносу'
        && matched?.id === `${text(photo?.photoId)}:op${index+1}`
        && text(matched?.data?.[13]).includes('[VERCEL-OCR]')
      ) {
        existingReady.push({ row: matched.data, rowNumber: matched.row });
      }
  ...
  return {rows,meta,existingReady,
    readyCount:meta.filter(x=>!x.duplicate&&x.status==='Готово к переносу').length + existingReady.length,
...
export function createCashJournalPipeline({sheets,spreadsheetId,now=()=>new Date()}={}) {
  ...
  async function transferReady(staged) {
    const existingReady = Array.isArray(staged?.existingReady) ? staged.existingReady : [];
    const hasNewReadyRows = Array.isArray(staged?.rows) && staged.rows.length > 0 && staged.startRow;
    if (!hasNewReadyRows && !existingReady.length) return {operations:0,ddsRows:0,recoveredOperations:0};
    const ready=[...existingReady];
    (Array.isArray(staged?.rows) ? staged.rows : []).forEach((row,index)=>{ if(text(row[12])==='Готово к переносу') ready.push({row,rowNumber:staged.startRow+index}); });
    ...
    return withGoogleSheetsLease({
      ...
      run:async()=>{
        ...
        const ddsLocations=new Map();
        for(let i=0;i<(ddsCommentsResp?.data?.values||[]).length;i++) {
          const id=ddsCashId(ddsCommentsResp.data.values[i]?.[0]);
          if(!id) continue;
          ddsCounts.set(id,(ddsCounts.get(id)||0)+1);
          if(!ddsLocations.has(id)) ddsLocations.set(id,[]);
          ddsLocations.get(id).push(DDS_FIRST_ROW+i);
        }
        const wallets=buildWallets(walletResp?.data?.values||[]);
        const activities=buildActivities(articleResp?.data?.values||[]);
        const ddsRows=[]; const source=[]; const recoveryLogRows=[];
        for(const item of ready) {
          ...
          if(transferred.has(id)||Number(ddsCounts.get(id)||0)>=expectedParts) {
            ...
            if(!transferred.has(id)) {
              const locations=ddsLocations.get(id)||[];
              if(locations.length===expectedParts) {
                const stamp=now() instanceof Date?now().toISOString():new Date(now()).toISOString();
                recoveryLogRows.push([
                  id,stamp,item.rowNumber,type,expectedParts,
                  Math.min(...locations),Math.max(...locations),
                  'Восстановлено — ДДС уже записан, audit trail дозакрыт'
                ]);
              }
            }
            continue;
        ...
        if(!ddsRows.length) {
          if(recoveryLogRows.length) {
            await values.append({spreadsheetId,range:`'${TRANSFER_LOG_SHEET}'!A:H`,valueInputOption:'RAW',
              insertDataOption:'INSERT_ROWS',requestBody:{values:recoveryLogRows}});
          }
          return {operations:0,ddsRows:0,recoveredOperations:recoveryLogRows.length};
        }
        ...
        const allLogRows=[...recoveryLogRows,...logRows];
        if(allLogRows.length) await values.append({spreadsheetId,range:`'${TRANSFER_LOG_SHEET}'!A:H`,valueInputOption:'RAW',insertDataOption:'INSERT_ROWS',requestBody:{values:allLogRows}});
        return {operations:source.length,ddsRows:ddsRows.length,recoveredOperations:recoveryLogRows.length,firstDDSRow:start,lastDDSRow:end};
      }
    });
  }

  async function reconcileDraftBacklog(limit = 100) {
    const max = Math.max(1, Math.min(500, Number(limit) || 100));
    const [draftResp, rulesResp, walletResp] = await Promise.all([
      values.get({spreadsheetId,range:`'${DRAFT_SHEET}'!A2:N`,valueRenderOption:'UNFORMATTED_VALUE'}),
      values.get({spreadsheetId,range:`'${RULES_SHEET}'!A2:F`,valueRenderOption:'UNFORMATTED_VALUE'}),
      values.get({spreadsheetId,range:`'${WALLETS_SHEET}'!A2:O`,valueRenderOption:'UNFORMATTED_VALUE'})
    ]);
    const rows = draftResp?.data?.values || [];
    const rules = buildRuleRows(rulesResp?.data?.values || []);
    const walletRows = walletResp?.data?.values || [];
    const wallets = buildWallets(walletRows);
    const checkpointPhotoByBranch = new Map();
    for (const walletRow of walletRows) {
      if (normalize(walletRow?.[2]) !== 'ФИЛИАЛ') continue;
      const name = text(walletRow?.[1]);
      const checkpointPhotoId = text(walletRow?.[14]);
      if (!name || !checkpointPhotoId) continue;
      checkpointPhotoByBranch.set(normalize(name.replace(/^Касса\s+/i,'')), checkpointPhotoId);
    }

    const claimedFingerprints = new Map();
    for (let index = 0; index < rows.length; index += 1) {
      const row = rows[index] || [];
      if (!text(row[12]).startsWith('Перенесено')) continue;
      const fp = fingerprint(row[1], row[2], row[4], row[5], row[6]);
      if (fp && !claimedFingerprints.has(fp)) claimedFingerprints.set(fp, index + 2);
    }
    ...
    const summary = {
      ...
      checkpointed: 0,
      unresolved: 0,
      transferredOperations: 0,
      createdDDSRows: 0
    };

    for (let index = 0; index < rows.length && summary.scanned < max; index += 1) {
      const row = rows[index] || [];
      const status = text(row[12]);
      if (status !== 'Проверить' && status !== 'Готово к переносу') continue;
      summary.scanned += 1;
      const rowNumber = index + 2;
      const id = text(row[0]);
      const checkpointPhotoId = checkpointPhotoByBranch.get(normalize(row[2]));
      if (checkpointPhotoId && id.startsWith(`${checkpointPhotoId}:`)) {
        const previousComment = text(row[13]);
        const checkpointComment = [
          previousComment,
          `[AUTO-RECHECK] Историческая строка включена в checkpoint ${checkpointPhotoId}; в ДДС повторно не переносится`
        ].filter(Boolean).join(' | ').slice(0, 45000);
        await values.update({
          spreadsheetId,
          range:`'${DRAFT_SHEET}'!M${rowNumber}:N${rowNumber}`,
          valueInputOption:'RAW',
          requestBody:{values:[['Checkpoint — уже учтено', checkpointComment]]}
        });
        summary.checkpointed += 1;
        continue;
      }
      const fp = fingerprint(row[1], row[2], row[4], row[5], row[6]);
      const claimedRow = fp ? claimedFingerprints.get(fp) : 0;
      if (claimedRow && claimedRow !== rowNumber) {
        await values.update({
          spreadsheetId,
          range:`'${DRAFT_SHEET}'!M${rowNumber}:N${rowNumber}`,
          valueInputOption:'RAW',
          requestBody:{values:[['Дубль — уже учтено',`[AUTO-RECHECK] Совпадает с уже проведённой строкой ${claimedRow}`]]}
        });
      ...
      if (status === 'Проверить') {
        const comment = text(row[13]);
        if (!comment.includes('[VERCEL-OCR]') || HARD_REVIEW_REASON.test(comment)) {
          summary.unresolved += 1;
          continue;
        }
        const next = reclassifyExistingDraftRow(row, rules, wallets);
        ...
        const nextComment = [
          comment,
          '[AUTO-RECHECK] Однозначно переклассифицировано по текущим правилам'
        ].filter(Boolean).join(' | ').slice(0, 45000);
        const nextRow = [...row];
        nextRow[7] = next.type;
        nextRow[8] = next.article;
        nextRow[9] = next.fromWallet;
        nextRow[10] = next.toWallet;
        nextRow[12] = 'Готово к переносу';
        nextRow[13] = nextComment;
        await values.update({
          spreadsheetId,
          range:`'${DRAFT_SHEET}'!H${rowNumber}:N${rowNumber}`,
          valueInputOption:'RAW',
          requestBody:{values:[[next.type,next.article,next.fromWallet,next.toWallet,row[11],'Готово к переносу',nextComment]]}
        });
        ready.push({row:nextRow,rowNumber});
        if (fp) claimedFingerprints.set(fp, rowNumber);
        summary.promoted += 1;
        continue;
      }

      ready.push({row:[...row],rowNumber});
      if (fp) claimedFingerprints.set(fp, rowNumber);
    }

    if (ready.length) {
      const transfer = await transferReady({existingReady:ready});
      summary.transferredOperations = Number(transfer?.operations || 0);
      summary.createdDDSRows = Number(transfer?.ddsRows || 0);
      summary.recovered += Number(transfer?.recoveredOperations || 0);
    }
    return summary;
  }

  return {
    async reconcileDraftBacklog(limit = 100) {
      return reconcileDraftBacklog(limit);
    },

    async syncRecognition(photo,data,{ allowTransfer = true } = {}) {
      const staged=await stage(photo,data);
      const transfer=allowTransfer
        ? await transferReady(staged)
        : { operations: 0, ddsRows: 0, recoveredOperations: 0 };
      return {
        ...
        recoveredOperations:transfer.recoveredOperations||0,
