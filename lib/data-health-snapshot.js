...
export function parseDataHealthSnapshot(rows = []) {
  ...
  const tochkaDdsQueueStructureRow = findRow(
    source,
    'Точка → ДДС',
    'структура очереди — сегодня + backlog'
  );
  ...
  if (tochkaDdsQueueStructureRow) {
    tochkaDds.queueStructureStatus = text(tochkaDdsQueueStructureRow?.[3]).toUpperCase();
  }
...
export function evaluateDataHealthSnapshot(snapshot = {}, thresholds = {}) {
  ...
  const queueStructureStatusPresent = Object.hasOwn(
    snapshot?.tochkaDds ?? {},
    'queueStructureStatus'
  );
  const queueStructureStatus = text(snapshot?.tochkaDds?.queueStructureStatus).toUpperCase();
  const ddsQueueStructureInconsistent =
    queueStructureStatusPresent && queueStructureStatus !== 'OK';
  if (ddsQueueStructureInconsistent) {
    consistencyErrors.push('tochka dds queue structure inconsistent');
  }
