...
} from './cash-photo-gateway-fallback.js';

const GEMINI_MODELS_URL = 'https://generativelanguage.googleapis.com/v1beta/models';
const TRANSIENT_STATUS = new Set([408, 429, 500, 502, 503, 504]);
export const DEFAULT_CASH_PHOTO_MODELS = Object.freeze(['gemini-3.5-flash-lite', 'gemini-3.6-flash']);
const PUBLIC_MESSAGE = 'Распознавание временно недоступно. Фото сохранено, повторно загружать его не нужно.';
const GATEWAY_DIRECT_TIMEOUT_MS = 18_000;
const GATEWAY_DIRECT_REQUEST_TIMEOUT_MS = 8_000;
...
async function gatewayFallback({ getGatewayToken, gatewayFetchImpl, payload }) {
  let gatewayToken;
  try {
    gatewayToken = await getGatewayToken();
  } catch {
    throw new CashPhotoRecognitionUnavailableError({ diagnostics: ['gateway OIDC unavailable'] });
  }
  return recognizeCashPhotoViaGateway({
    gatewayToken,
    payload,
    fetchImpl: gatewayFetchImpl
  });
}
...
export async function recognizeWithFallback({
  ...
  gatewayEnabled = Boolean(process.env.VERCEL),
  getGatewayToken = getVercelOidcToken,
  gatewayFetchImpl = globalThis.fetch
} = {}) {
  ...
  if (!validModels(models) || !Number.isInteger(maxAttemptsPerModel) || maxAttemptsPerModel < 1 || maxAttemptsPerModel > 3
      ...
      || (gatewayEnabled && (typeof getGatewayToken !== 'function' || typeof gatewayFetchImpl !== 'function'))) {
    throw permanentFailure('INVALID_CONFIGURATION');
  }

  const directTotalTimeoutMs = gatewayEnabled
    ? Math.min(totalTimeoutMs, GATEWAY_DIRECT_TIMEOUT_MS)
    : totalTimeoutMs;
  const directRequestTimeoutMs = gatewayEnabled
    ? Math.min(requestTimeoutMs, GATEWAY_DIRECT_REQUEST_TIMEOUT_MS)
    : requestTimeoutMs;
  const deadline = now() + directTotalTimeoutMs;
  const diagnostics = [];
  modelLoop: for (const model of models) {
    for (let attempt = 1; attempt <= maxAttemptsPerModel; attempt++) {
      ...
      const result = await requestJson(`${GEMINI_MODELS_URL}/${model}:generateContent`, {
        apiKey: key, payload, fetchImpl, timeoutMs: Math.min(directRequestTimeoutMs, remaining)
      });
  ...
  const directError = new CashPhotoRecognitionUnavailableError({ diagnostics });
  if (!gatewayEnabled) throw directError;
  return recognizeCashPhotoWithGatewayFallback({
    primaryRecognize: async () => { throw directError; },
    gatewayRecognize: () => gatewayFallback({ getGatewayToken, gatewayFetchImpl, payload })
  });
}
