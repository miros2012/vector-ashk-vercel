    ...
    if (stage === 'payments') {
      if (probe === 'payment-record-module') {
        try {
          ...
          console.log(JSON.stringify({ event: 'ashk-payment-record-module-probe', result }));
          return res.status(200).json({ ok: true, probe: 'payment-record-module', result });
        } catch (error) {
          console.error('ashk-payment-record-module-probe:', error?.name || 'Error');
      ...
      if (probe === 'admin-kpi-report') {
        try {
          ...
          console.log(JSON.stringify({ event: 'ashk-admin-kpi-report-probe', result }));
          return res.status(200).json({ ok: true, probe: 'admin-kpi-report', result });
        } catch (error) {
          console.error('ashk-admin-kpi-report-probe:', error?.name || 'Error');
      ...
      if (probe === 'report-keywords') {
        try {
          ...
          console.log(JSON.stringify({ event: 'ashk-report-keyword-probe', result }));
          return res.status(200).json({ ok: true, probe: 'report-keywords', result });
        } catch (error) {
          console.error('ashk-report-keyword-probe:', error?.name || 'Error');
