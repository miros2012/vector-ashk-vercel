...
const CONTROL_SHEET = 'Контроль Точка → ДДС';
...
function validateControlHeader(values) {
  const header = Array.isArray(values?.[0]) ? values[0] : [];
  for (let index = 0; index < CONTROL_HEADERS.length; index += 1) {
    if (text(header[index]) !== CONTROL_HEADERS[index]) {
      throw new Error(`Tochka control header mismatch at column ${index + 1}`);
    }
  }
}
...
function ddsKeyFromComment(value) {
  ...
  if (start < 0) return '';
  const key = comment.slice(start + prefix.length).split(/ \| |[\r\n]/, 1)[0].trim();
...
// A durable accounting decision is different from a lost import. Keep it in
// the existing journal so recovery cannot erase the owner's manual split.
// Row references are deliberately fail-closed: if somebody moves/changes the
// replacement rows, reconcile the evidence before allowing another import.
function manualReplacements(journalValues) {
  const decisions = new Map();
  const counts = journalKeyCounts(journalValues);
  const usedRows = new Set();
  for (const row of journalValues || []) {
    const status = text(row?.[4]);
    if (!status.startsWith('Учтено вручную')) continue;
    const key = text(row?.[0]);
    let decision;
    try {
      if (!status.startsWith('Учтено вручную | ')) throw new Error();
      decision = JSON.parse(status.slice('Учтено вручную | '.length));
    } catch {
      throw new Error('Invalid manual replacement evidence');
    }
    if (!key || !text(row?.[1]) || !key.endsWith(`|${text(row[1])}`)
      || counts.get(key) !== 1 || !decision || !text(decision.reason)
      || !Array.isArray(decision.rows) || !decision.rows.length || decision.rows.length > 100
      || typeof decision.amount !== 'number' || !Number.isFinite(decision.amount) || decision.amount === 0
      || !Number.isInteger(decision.wallet) || decision.wallet < 1
      || !/^\d{4}-\d{2}-\d{2}$/.test(text(decision.date))) {
      throw new Error('Invalid manual replacement evidence');
    }
    for (const rowNumber of decision.rows) {
      if (!Number.isInteger(rowNumber) || rowNumber < DDS_FIRST_DATA_ROW || rowNumber > DDS_LAST_DATA_ROW
        || usedRows.has(rowNumber)) throw new Error('Invalid manual replacement row reference');
...
function verifyManualReplacements(decisions, replacementDdsRows, existingDds) {
  for (const [key, decision] of decisions) {
    let cents = 0;
    let referencedKeyCount = 0;
    for (const rowNumber of decision.rows) {
      const row = replacementDdsRows[rowNumber];
      const amount = numberValue(row?.[3]);
      if (!Array.isArray(row) || !text(row[3]) || amount === null || amount === 0
        || Math.sign(amount) !== Math.sign(decision.amount)
        || numberValue(row[4]) !== decision.wallet || normalizedDate(row[2]) !== decision.date
        || !text(row[8]) || !text(row[10])) {
        throw new Error('Manual replacement DDS evidence mismatch');
      }
      cents += Math.round(amount * 100);
      if (ddsKeyFromComment(row[12]) === key) referencedKeyCount += 1;
    }
    if (cents !== Math.round(decision.amount * 100)) {
      throw new Error('Manual replacement DDS amount mismatch');
    }
    if ((existingDds.get(key) || 0) !== referencedKeyCount) {
      throw new Error('Manual replacement conflicts with imported DDS row');
    }
  }
}

function recoveryReadyRows({ controlValues, existingDds, existingJournal, businessDate }) {
  if (!Array.isArray(controlValues) || controlValues.length === 0) return [];
  validateControlHeader(controlValues);
  const rows = [];

  for (let index = 1; index < controlValues.length; index += 1) {
    const row = controlValues[index];
    if (!Array.isArray(row) || text(row[15]) !== 'Импортировано ранее') continue;
    const rowDate = normalizedDate(row[0]);
    const key = text(row[10]);
    if (!rowDate || rowDate > businessDate || !key) continue;
    if (Number(existingDds.get(key) || 0) !== 0) continue;
    if (Number(existingJournal.get(key) || 0) < 1) continue;

    const month = Number(rowDate.slice(5, 7));
    ...
    rows.push([
      ...
      row[7],
      row[12],
      text(row[3]) === 'Приход' ? 'Поступление' : 'Выбытие',
      row[13],
      row[14],
      `Точка API | ${key}`,
      key,
...
export function buildCurrentDayTochkaDdsPlan({
  ...
  const replacements = manualReplacements(journalValues);
  verifyManualReplacements(replacements, replacementDdsRows, existingDds);
  const readyRows = Array.isArray(readyValues) ? readyValues.slice(1) : [];
  const recoveryRows = recoveryReadyRows({
    controlValues,
    existingDds,
    existingJournal,
    businessDate: date
  });
  const candidateRows = [...readyRows, ...recoveryRows];
  const eligibleKeys = [];
  const ddsRows = [];
  const journalRows = [];
  const seen = new Set();
  ...
  for (let index = 0; index < candidateRows.length; index += 1) {
    ...
    const replacement = replacements.get(rawKey);
    if (replacement) {
      if (rowDate !== replacement.date
        || Math.round(numberValue(row[3]) * 100) !== Math.round(replacement.amount * 100)
        || numberValue(row[4]) !== replacement.wallet) {
        throw new Error('Manual replacement source mismatch');
      }
      continue;
    }
    ...
    let normalized;
    try {
      normalized = validateCurrentDayRow(row, index + 2, rowDate, importedAt);
    } catch (error) {
...
async function syncCurrentDayTochkaDdsUnlocked({
  ...
  const ranges = [
    ...
    `'${CONTROL_SHEET}'!A4:P3000`
  ];
  const snapshot = await boundedGoogleSheetsRequest(options => valuesApi.batchGet({
    spreadsheetId: id,
    ranges,
    valueRenderOption: 'UNFORMATTED_VALUE'
  }, options), requestTimeoutMs, 'snapshot');
  const valueRanges = snapshot?.data?.valueRanges || [];
  const replacements = manualReplacements(valueRanges[2]?.values || []);
  const replacementRows = [...replacements.values()].flatMap(item => item.rows);
  const replacementDdsRows = {};
  // Bound each request rather than rereading the entire financial register.
  for (let offset = 0; offset < replacementRows.length; offset += 100) {
    const rows = replacementRows.slice(offset, offset + 100);
    const evidence = await boundedGoogleSheetsRequest(options => valuesApi.batchGet({
      spreadsheetId: id,
      ranges: rows.map(row => `'${DDS_SHEET}'!A${row}:M${row}`),
      valueRenderOption: 'UNFORMATTED_VALUE'
    }, options), requestTimeoutMs);
    rows.forEach((row, index) => { replacementDdsRows[row] = evidence?.data?.valueRanges?.[index]?.values?.[0]; });
  }
  const plan = buildCurrentDayTochkaDdsPlan({
    readyValues: valueRanges[0]?.values || [],
    controlValues: valueRanges[4]?.values || [],
  ...
  const ddsReadback = await boundedGoogleSheetsRequest(options => valuesApi.get({
    ...
    valueRenderOption: 'UNFORMATTED_VALUE'
  }, options), requestTimeoutMs, 'dds-readback');
  verifyKeys({
  ...
  return async function tochkaDdsImportHandler(req, res) {
    ...
    try {
      ...
      if (!result?.ok || result?.verified !== true) {
        return res.status(502).json({ ok: false, errorClass:'DDS_IMPORT_FAILED',error: 'Tochka DDS import verification failed' });
      }
      ...
      const errorClass=transient?'DDS_TRANSPORT':'DDS_IMPORT_FAILED';
      console.error(JSON.stringify({event:'tochka-dds-import-failure',errorClass,
        ...(['lease-read','lease-metadata','lease-cas','snapshot','dds-readback'].includes(error?.phase)?{phase:error.phase}:{}),
        ...(Number.isInteger(status)&&status>=400&&status<=599?{status}:{})}));
      return res.status(transient?503:502).json({ ok: false,errorClass,error: 'Tochka DDS import failed' });
    }
  };
}
