...
const READY_HEADERS = [
  'Месяц','Мсц','Дата','Сумма','Кошелек','Направление','Контрагент','Назначение',
  'Статья','Платеж/поступл','Вид д-ти','Месяц P&L','Комментарий P&L','Ключ дубля','transactionId'
];
...
function verifyKeys({ counts, keys, newlyAppended, label }) {
  const appended = new Set(newlyAppended);
  for (const key of keys) {
    const count = Number(counts.get(key) || 0);
    if (count < 1 || (appended.has(key) && count !== 1)) {
...
export async function syncCurrentDayTochkaDds({
  ...
  const ddsAppendedKeys = plan.ddsRows.map(row => ddsKeyFromComment(row[12]));
  ...
  const journalAppendedKeys = journalRows.map(row => text(row[0]));
  ...
  verifyKeys({
    ...
    keys: plan.eligibleKeys,
    newlyAppended: journalAppendedKeys,
    label: 'Journal'
  });
...
export function createTochkaDdsImportHandler({ cronSecret, runImport } = {}) {
  ...
  return async function tochkaDdsImportHandler(req, res) {
    ...
    try {
      ...
      console.error('tochka-dds-current-day:', error?.name || 'Error');
