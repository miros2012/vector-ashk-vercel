...
function lastOccupiedRow(values, startRow) {
  const rows = Array.isArray(values) ? values : [];
  for (let index = rows.length - 1; index >= 0; index -= 1) {
    if (Array.isArray(rows[index]) && rows[index].some(value => text(value) !== '')) {
...
async function nextSafeDdsRow(valuesApi, spreadsheetId) {
  const [canonical, shifted] = await Promise.all([
    valuesApi.get({
      spreadsheetId,
      range: `'${DDS_SHEET}'!A${DDS_FIRST_DATA_ROW}:A${DDS_LAST_DATA_ROW}`,
      valueRenderOption: 'UNFORMATTED_VALUE'
    }),
    valuesApi.get({
      spreadsheetId,
      range: `'${DDS_SHEET}'!S${DDS_FIRST_DATA_ROW}:S${DDS_LAST_DATA_ROW}`,
      valueRenderOption: 'UNFORMATTED_VALUE'
    })
  ]);
  const lastRow = Math.max(
    lastOccupiedRow(canonical?.data?.values || [], DDS_FIRST_DATA_ROW),
    lastOccupiedRow(shifted?.data?.values || [], DDS_FIRST_DATA_ROW)
  );
  return Math.max(lastRow + 1, DDS_FIRST_DATA_ROW);
}
...
async function syncCurrentDayTochkaDdsUnlocked({
  ...
  if (!valuesApi?.batchGet || !valuesApi?.append || !valuesApi?.update || !valuesApi?.get) {
  ...
  if (plan.ddsRows.length) {
    const startRow = await nextSafeDdsRow(valuesApi, id);
    const endRow = startRow + plan.ddsRows.length - 1;
    if (endRow > DDS_LAST_DATA_ROW) throw new Error('DDS write range exhausted');
