...
function phaseCode(phase) {
  return `TOCHKA_DDS_${text(phase).toUpperCase().replace(/[^A-Z0-9]+/g, '_') || 'FAILED'}`;
}

function wrapPhaseError(error, phase) {
  const wrapped = new Error(error instanceof Error ? error.message : 'Tochka DDS import failed');
  wrapped.name = error?.name || 'Error';
  wrapped.code = error?.code || phaseCode(phase);
  wrapped.cause = error;
  return wrapped;
}
...
export function findNextDdsRow(anchorValues, startRow = DDS_FIRST_DATA_ROW) {
  ...
  if (!Number.isInteger(firstRow) || firstRow < 1) throw new Error('startRow must be a positive integer');
  return firstRow + (Array.isArray(anchorValues) ? anchorValues.length : 0);
}
...
export async function syncCurrentDayTochkaDds({
  sheets,
  spreadsheetId,
  businessDate,
  now = () => new Date()
  ...
  try {
    ...
    const ranges = [
      ...
      `'${DDS_SHEET}'!M${DDS_FIRST_DATA_ROW}:M${DDS_LAST_ROW}`,
      `'${JOURNAL_SHEET}'!A2:E3000`,
      `'${DDS_SHEET}'!A${DDS_FIRST_DATA_ROW}:A${DDS_LAST_ROW}`
    ];
    ...
    if (plan.ddsRows.length) {
      if (typeof valuesApi.update !== 'function') throw new Error('sheets values update client is required');
      const ddsStartRow = findNextDdsRow(valueRanges[3]?.values || [], DDS_FIRST_DATA_ROW);
      const ddsEndRow = ddsStartRow + plan.ddsRows.length - 1;
      if (ddsEndRow > DDS_LAST_ROW) throw new Error('DDS target capacity exceeded');

      phase = 'dds-preflight';
      const targetRead = await valuesApi.get({
        spreadsheetId: id,
        range: `'${DDS_SHEET}'!A${ddsStartRow}:S${ddsEndRow}`,
        valueRenderOption: 'UNFORMATTED_VALUE'
      });
      if (hasAnyValue(targetRead?.data?.values || [])) {
    ...
    const ddsReadback = await valuesApi.get({
      spreadsheetId: id,
      range: `'${DDS_SHEET}'!M${DDS_FIRST_DATA_ROW}:M${DDS_LAST_ROW}`,
      valueRenderOption: 'UNFORMATTED_VALUE'
    });
    verifyKeys({
    ...
    throw wrapPhaseError(error, phase);
  }
}
...
export function createTochkaDdsImportHandler({ cronSecret, runImport } = {}) {
  ...
  return async function tochkaDdsImportHandler(req, res) {
    ...
    try {
      ...
      console.error('tochka-dds-current-day:', error?.code || error?.name || 'Error');
