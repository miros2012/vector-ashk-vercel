...
export function createAshkWebSession({ baseUrl, login, password, fetchFn = fetch }) {
  ...
  async function requestText(path, params = {}, retried = false) {
    if (!cookie) await authenticate();
    const url = new URL(path, baseUrl);
    for (const [key, value] of Object.entries(params)) {
      if (value !== undefined && value !== null && value !== '') {
    ...
    if ((response.status === 401 || response.status === 403) && !retried) {
      cookie = '';
      await authenticate();
      return requestText(path, params, true);
    }
    if (!response.ok) throw new Error(`ASHK web request failed: ${response.status}`);
    return response.text();
  }

  return { authenticate, requestJson, requestText };
}
