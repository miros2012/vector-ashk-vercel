...
export const PAYMENT_PERIOD_CANDIDATES = Object.freeze([
  Object.freeze({ name: 'period-month', staticParams: Object.freeze({ Period: 'Month' }) }),
  Object.freeze({ name: 'period-custom-start-date', staticParams: Object.freeze({ Period: 'Custom' }), fromKey: 'StartDate', toKey: 'EndDate' }),
  Object.freeze({ name: 'period-custom-pay-date', staticParams: Object.freeze({ Period: 'Custom' }), fromKey: 'PayDateFrom', toKey: 'PayDateTo' }),
  Object.freeze({ name: 'period-custom-date', staticParams: Object.freeze({ Period: 'Custom' }), fromKey: 'DateFrom', toKey: 'DateTo' }),
  Object.freeze({ name: 'period-custom-start-finish', staticParams: Object.freeze({ Period: 'Custom' }), fromKey: 'Start', toKey: 'Finish' }),
...
function summarizeCandidate(candidate, payload) {
  ...
  return {
    name: candidate.name,
    staticParams: candidate.staticParams || undefined,
...
export async function probeAshkPaymentPeriodCandidates({ session, startDate, endDate } = {}) {
  ...
  for (const candidate of PAYMENT_PERIOD_CANDIDATES) {
    try {
      ...
      if (candidate.fromKey) params[candidate.fromKey] = startDate;
      if (candidate.toKey) params[candidate.toKey] = endDate;
      const payload = await session.requestJson('/api/PaymentRecordDebitList', params);
