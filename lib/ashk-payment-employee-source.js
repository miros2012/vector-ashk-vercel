...
export function createAshkPaymentEmployeeSource({ session, pageSize = 200, maxPages = 100 } = {}) {
  ...
  async function fetchPeriod({ startDate, endDate } = {}) {
    ...
    const payDates = rows.map(row => text(row?.PayDate)).filter(Boolean).sort();
    return {
      rows,
      metrics: {
        rows: rows.length,
        ...
        pages,
        debitTotal: roundMoney(rows.reduce((sum, row) => sum + toNumber(row?.Debit), 0)),
        minPayDate: payDates[0] || '',
        maxPayDate: payDates.at(-1) || ''
      }
    };
  }
