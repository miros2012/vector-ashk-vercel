import { CashPhotoRecognitionUnavailableError } from './cash-photo-recognizer.js';

export function createCashPhotoRetryService({ store, recognize } = {}) {
  if (!store?.listPending || !store?.readPhoto) throw new Error('Cash photo store with retry support is required');
  if (typeof recognize !== 'function') throw new Error('Cash photo recognizer is required');

  async function retryPhoto(photo) {
    try {
      const downloaded = await store.readPhoto(photo);
      await store.markRecognizing?.(photo);
      const recognition = await recognize({
        imageBytes: downloaded.imageBytes,
        mimeType: downloaded.mimeType,
        branch: photo.branch,
        year: photo.year,
        fileName: photo.fileName
      });
      await store.markRecognized?.(photo, recognition);
      return 'recognized';
    } catch (error) {
      if (error instanceof CashPhotoRecognitionUnavailableError || error?.retryable === true) {
        await store.markPending?.(photo, {
          message: error.publicMessage || 'Распознавание временно недоступно. Фото сохранено.',
          diagnostics: Array.isArray(error.diagnostics) ? error.diagnostics : []
        });
        return 'stillPending';
      }
      await store.markFailed?.(photo, {
        message: 'Автоматическое распознавание не выполнено. Фото сохранено.',
        diagnostics: Array.isArray(error?.diagnostics) ? error.diagnostics : [String(error?.message || error)]
      });
      return 'failed';
    }
  }

  function summarize(outcomes) {
    const result = {
      attempted: outcomes.length,
      recognized: 0,
      stillPending: 0,
      failed: 0
    };
    for (const outcome of outcomes) result[outcome] += 1;
    return result;
  }

  return {
    async retryPending(limit = 3, filters = {}) {
      const photos = await store.listPending(limit, filters);
      const outcomes = [];
      for (const photo of photos) outcomes.push(await retryPhoto(photo));
      return summarize(outcomes);
    },

    async retryPendingConcurrent(limit = 3, filters = {}, options = {}) {
      const photos = await store.listPending(limit, filters);
      const outcomes = await Promise.all(photos.map(photo => retryPhoto(photo)));
      const result = summarize(outcomes);
      if (typeof store.syncRecognizedDraftBacklog === 'function') {
        try {
          result.draftBackfill = await store.syncRecognizedDraftBacklog(1);
        } catch (error) {
          console.error('cash journal draft backfill failed', error?.name || 'Error');
          result.draftBackfill = { attempted: 0, synced: 0, skipped: 0, failed: 1 };
        }
      }
      if (options?.reconcileDraft !== false && typeof store.reconcileDraftBacklog === 'function') {
        try {
          result.draftReconciliation = await store.reconcileDraftBacklog(20);
        } catch (error) {
          console.error('cash journal draft reconciliation failed', error?.name || 'Error');
          result.draftReconciliation = {
            scanned: 0,
            promoted: 0,
            recovered: 0,
            duplicates: 0,
            unresolved: 0,
            transferredOperations: 0,
            createdDDSRows: 0,
            failed: 1
          };
        }
      }
      return result;
    }
  };
}
