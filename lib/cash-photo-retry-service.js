...
export function createCashPhotoRetryService({ store, recognize } = {}) {
  ...
  return {
    ...
    async retryPendingConcurrent(limit = 3, filters = {}) {
      const photos = await store.listPending(limit, filters);
      ...
      if (typeof store.reconcileDraftBacklog === 'function') {
        try {
          result.draftReconciliation = await store.reconcileDraftBacklog(100);
        } catch (error) {
