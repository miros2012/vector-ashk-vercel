...
export function filterRowsByFactPeriod(rows, startDate, endDate) {
  const start = String(startDate || '').slice(0, 10);
  const end = String(endDate || '').slice(0, 10);
  if (!start || !end) return rows;
  return rows.filter((row) => {
    const factStart = typeof row?.FactStart === 'string' ? row.FactStart : '';
    const factDate = factStart.slice(0, 10);
    return /^\d{4}-\d{2}-\d{2}$/.test(factDate) && factDate >= start && factDate <= end;
  });
}

export function canonicalizeMasterReportRows(rows, loadedAt = new Date().toISOString()) {
  const canonicalRows = [];
  const seen = new Set();

  for (const row of rows || []) {
    const factStart = typeof row?.FactStart === 'string' ? row.FactStart.trim() : '';
    const factDate = factStart.slice(0, 10);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(factDate)) continue;

    const employeeId = String(row?.EmployeeId ?? '').trim();
    const masterName = String(row?.MasterName ?? '').trim();
    const contractName = String(row?.ContractName ?? '').trim();
    const sessionTypeName = String(row?.SessionTypeName ?? '').trim();
    const hours = finiteNumber(row?.Hours);
    const parallelHours = finiteNumber(row?.ParallelHours);
    const visitState = finiteNumber(row?.VisitState);
    const mainProductName = String(row?.MainProductName ?? '').trim();
