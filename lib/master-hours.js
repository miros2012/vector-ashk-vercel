...
export function summarizeMasterHours(rows) {
  ...
  for (const row of rows) {
    ...
    const factDate = /^\d{4}-\d{2}-\d{2}/.test(factStart || '') ? factStart.slice(0, 10) : null;
    if (factDate) {
      const dateBucket = summary.byFactDate[factDate] || { rows: 0, hours: 0, parallelHours: 0 };
      dateBucket.rows += 1;
      dateBucket.hours += hours;
      dateBucket.parallelHours += parallelHours;
      summary.byFactDate[factDate] = dateBucket;
    }
  ...
  summary.byFactDate = Object.fromEntries(Object.entries(summary.byFactDate).sort());
