import { verifyGoogleSheetsFinanceReports } from './google-sheets-finance-reports.js';
import { cycleFromControlRows } from './google-sheets-finance-cycle-store.js';
import { verifiedFinanceCycleHealth } from './finance-cycle-status.js';
...
import {
  ...
  isUnconfirmedObligationStatus
} from './obligation-status.js';

const BUSINESS_TIME_ZONE = 'Asia/Yekaterinburg';
const OWNER_OPERATING_RESERVE_RANGE = "'Настройки системы'!A1:C40";
...
export async function readOwnerOperatingReserve({ sheets, spreadsheetId }) {
  if (!sheets?.spreadsheets?.values?.get) {
    throw new Error('owner operating reserve read-only Sheets client is required');
  }
  if (!text(spreadsheetId)) throw new Error('owner operating reserve spreadsheetId is required');

  const response = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: OWNER_OPERATING_RESERVE_RANGE,
    valueRenderOption: 'UNFORMATTED_VALUE'
  });
  const rows = response?.data?.values;
  if (!Array.isArray(rows)) return undefined;

  const matches = rows.filter(row => text(row?.[0]) === 'Операционный резерв');
...
export function createOwnerLiveSourceReader({ sheets, spreadsheetId, now = () => new Date() }) {
  ...
  return {
    async readOwnerLiveFacts() {
      ...
      const health = await runOwnerPackageStage('DATA_HEALTH_PARSE', async () => {
        const snapshot = parseDataHealthSnapshot(matrices.dataHealth);
        const evaluation = evaluateDataHealthSnapshot(snapshot);
        const cycle = await verifiedFinanceCycleHealth(cycleFromControlRows(matrices.financeControl), {now:now()},
          fingerprint => verifyGoogleSheetsFinanceReports({sheets,spreadsheetId},fingerprint));
        if (!cycle.ok) {
          evaluation.ok = false; evaluation.status = 'BLOCKED';
          evaluation.consistencyErrors = [...evaluation.consistencyErrors, cycle.reason];
          evaluation.staleCoreSources = [...evaluation.staleCoreSources, 'financeCycle'];
        }
        return {snapshot, evaluation: {...evaluation, cycle}};
      });
