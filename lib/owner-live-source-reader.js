...
function parseReceivables(values) {
  ...
  return {
    contracts,
    debt: requiredFinite(row[3], 'owner live receivables debt'),
    sales: requiredFinite(row[4], 'owner live receivables sales'),
    paid: requiredFinite(row[5], 'owner live receivables paid')
  };
}
...
function parseDrivingFund(values) {
  ...
  const valueFor = label => {
    ...
    return requiredFinite(row[9], `owner live driving fund ${label}`);
  };
...
export function createOwnerLiveSourceReader({ sheets, spreadsheetId, now = () => new Date() }) {
  ...
  return {
    async readOwnerLiveFacts() {
      ...
      const matrices = matricesFromBatch(batchResponse);
      const dataHealthSnapshot = parseDataHealthSnapshot(matrices.dataHealth);
      const dataHealth = evaluateDataHealthSnapshot(dataHealthSnapshot);

      return deepFreeze({
        businessDate,
        forecast: forecastFacts,
        dataHealth: { ...dataHealth, snapshot: dataHealthSnapshot },
        sales: parseSales(matrices.sales, businessDate),
        receivables: parseReceivables(matrices.receivables),
        obligations: parseObligations(matrices.obligations, matrices.adjustments),
        drivingFund: parseDrivingFund(matrices.drivingFund),
        decisions: parseDecisions(matrices.decisions),
        history: parseHistory(matrices.history)
      });
    }
  };
}
