...
function requireDailyCell(row, rowIndex, columnIndex, field) {
  if (!Array.isArray(row)) {
    throw new Error(`dailyRows[${rowIndex}] must be an array`);
  }
  if (columnIndex >= row.length || row[columnIndex] === undefined) {
    throw new Error(`dailyRows[${rowIndex}].${field} is missing`);
  }
...
export function parseOwnerForecastSheetValues(rows) {
  ...
  const parsed = dailyRows.map((row, index) => {
    const date = sheetSerialToIso(
      requireDailyCell(row, index, header.indexes['Дата'], 'date')
    );
    return {
      date,
      openingCash: finiteNumber(
        requireDailyCell(row, index, header.indexes['Остаток на начало'], 'openingCash'),
        `dailyRows[${index}].openingCash`
      ),
      inflow: nonNegativeNumber(
        requireDailyCell(row, index, header.indexes['Поступления всего'], 'inflow'),
        `dailyRows[${index}].inflow`
      ),
      outflow: nonNegativeNumber(
        requireDailyCell(row, index, header.indexes['Выплаты всего'], 'outflow'),
        `dailyRows[${index}].outflow`
      ),
      reserve: nonNegativeNumber(
        requireDailyCell(row, index, header.indexes['Резерв / блокировка'], 'reserve'),
        `dailyRows[${index}].reserve`
      )
    };
  });
