...
export function extractPaymentRecordDebitListHints(source) {
  ...
  const keys = [];
  const objectKeyPattern = /(?:return\s*)?\{([\s\S]{0,2400}?)\}/gi;
  let objectMatch;
  while ((objectMatch = objectKeyPattern.exec(moduleText))) {
    const keyPattern = /(?:^|,)\s*([A-Za-z_$][A-Za-z0-9_$]*)\s*:/g;
    let keyMatch;
    while ((keyMatch = keyPattern.exec(objectMatch[1]))) keys.push(keyMatch[1]);
  }
  return {
    found: true,
    candidateKeys: uniqueSorted(keys),
