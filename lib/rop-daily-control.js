...
function plannedManagerResolver(plans) {
  const byExact = new Map();
  const bySurname = new Map();
  const ambiguousSurnames = new Set();
  for (const plan of plans) {
    const exact = personKey(plan.manager);
    if (!exact) continue;
    byExact.set(exact, plan.manager);
    const surname = exact.split(' ')[0];
    if (bySurname.has(surname) && bySurname.get(surname) !== plan.manager) {
      ambiguousSurnames.add(surname);
    } else {
      bySurname.set(surname, plan.manager);
    }
  }
  return owner => {
    const exact = personKey(owner);
    if (!exact) return '';
    if (byExact.has(exact)) return byExact.get(exact);
    const surname = exact.split(' ')[0];
    if (ambiguousSurnames.has(surname)) return '';
...
function currentMonthContractValues(contracts, month) {
  return [CONTRACT_HEADERS, ...contracts
    ...
    .map(contract => [contract.studentId, contract.contractDate, contract.branch, contract.ashkBranch,
      contract.manager, contract.salesSum, contract.debitSum, contract.debt, contract.state, contract.contractName])];
}

export function buildCurrentMonthContractStaging({ groups, contractsByGroup, planValues, month }) {
  return currentMonthContractValues(flattenContracts(groups, contractsByGroup, parsePlans(planValues)), month);
}

function paymentRows(values) {
  const rows = Array.isArray(values) ? values : [];
  const headers = Array.isArray(rows[0]) ? rows[0].map(text) : [];
  const paymentEmployeeIndex = headers.indexOf('PaymentEmployeeName');
  const saleEmployeeIndex = headers.indexOf('SaleEmployeeName');
  const saleAttributionStatusIndex = headers.indexOf('SaleAttributionStatus');
  if (paymentEmployeeIndex < 0) {
    throw new Error('Payment staging missing PaymentEmployeeName header');
  }
  return rows
    .slice(1)
    ...
    .map(row => ({
      ...
      paymentEmployee: text(row[paymentEmployeeIndex]),
      saleEmployee: saleEmployeeIndex >= 0 ? text(row[saleEmployeeIndex]) : '',
      saleAttributionStatus: saleAttributionStatusIndex >= 0 ? text(row[saleAttributionStatusIndex]) : ''
    }));
}
...
const CONTROL_HEADERS = [
  ...
  'Личный факт за день','Личный факт с начала месяца','Выполнение личного плана',
...
const PAYMENT_ATTRIBUTION_HEADERS = [
  ...
  'Менеджер АШК','Сотрудник, принявший/выставивший оплату АШК','Сотрудник продажи АШК',
...
export function buildRopDailyControlWorkbook({
  ...
  const resolvePlannedManager = plannedManagerResolver(plans);
  ...
  for (const payment of paymentRows(paymentValues)) {
    ...
    const creditedManager = resolvePlannedManager(payment.paymentEmployee);
    const attributionStatus = !payment.paymentEmployee
      ? 'PAYMENT_EMPLOYEE_EMPTY'
      : (creditedManager ? 'OK_PAYMENT_EMPLOYEE' : 'PAYMENT_EMPLOYEE_NOT_IN_ACTIVE_PLAN');
    paymentAttributionRows.push([
      payment.id,
      payment.date,
      payment.studentId,
      payment.saleId,
  ...
  const managerDaily = sumByDate(
    resolvedPayments.filter(item => item.manager && item.amount > 0),
    'manager'
  );
  const controlRows = [];
  ...
  for (let day = 1; day <= maxDay; day += 1) {
    ...
    for (const plan of plans) {
      ...
      const noteParts = [];
      if (!singleManager) noteParts.push('Общий филиальный план; личный факт считается по сотруднику, принявшему/выставившему оплату в АШК.');
  ...
  return {
    currentMonthContractsValues: currentMonthContractValues(currentMonthContracts, month),
    controlValues: [CONTROL_HEADERS, ...controlRows],
    unmatchedPaymentValues: [UNMATCHED_PAYMENT_HEADERS, ...unmatchedPaymentRows],
    paymentAttributionValues: [PAYMENT_ATTRIBUTION_HEADERS, ...paymentAttributionRows],
    metrics: {
      currentMonthContracts: currentMonthContracts.length,
      resolvedPayments: resolvedPayments.length,
      managerAttributedPayments: resolvedPayments.filter(payment => payment.manager).length,
      managerUnattributedPayments: resolvedPayments.filter(payment => !payment.manager).length,
      managerUnattributedAmount: roundMoney(resolvedPayments
        .filter(payment => !payment.manager)
