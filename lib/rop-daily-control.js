...
function paymentRows(values) {
  ...
  return rows
    ...
    .map(row => ({
      ...
      paymentEmployee: paymentEmployeeIndex >= 0 ? text(row[paymentEmployeeIndex]) : null
    }));
}
...
const PAYMENT_ATTRIBUTION_HEADERS = [
  'ID оплаты','Дата','StudentId','Сумма','Филиал','Филиал АШК','Менеджер АШК',
  'Сотрудник оплаты АШК','Зачтён менеджеру','Статус привязки'
];
...
export function buildRopDailyControlWorkbook({
  ...
  for (const payment of paymentRows(paymentValues)) {
    ...
    const creditedManager = resolvePlannedManager(
      payment.paymentEmployee === null ? contract.manager : payment.paymentEmployee
    );
    paymentAttributionRows.push([
      ...
      payment.paymentEmployee === null
        ? (creditedManager ? 'LEGACY_OWNER_FALLBACK' : (contract.manager ? 'OWNER_NOT_IN_ACTIVE_PLAN' : 'OWNER_EMPTY'))
        : (creditedManager ? 'OK_PAYMENT_EMPLOYEE' : (payment.paymentEmployee ? 'PAYMENT_EMPLOYEE_NOT_IN_ACTIVE_PLAN' : 'PAYMENT_EMPLOYEE_EMPTY'))
    ]);
  ...
  for (let day = 1; day <= maxDay; day += 1) {
    ...
    for (const plan of plans) {
      ...
      const noteParts = [];
      if (!singleManager) noteParts.push('Общий филиальный план; личный факт считается по сотруднику, выставившему счёт/принявшему оплату в АШК.');
