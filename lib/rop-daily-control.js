...
function paymentRows(values) {
  ...
  if (saleEmployeeIndex < 0 || saleAttributionStatusIndex < 0) {
    throw new Error('Payment staging missing sale attribution headers');
  }
  return rows
    .slice(1)
    ...
    .map(row => ({
      ...
      paymentEmployee: paymentEmployeeIndex >= 0 ? text(row[paymentEmployeeIndex]) : '',
      saleEmployee: text(row[saleEmployeeIndex]),
      saleAttributionStatus: text(row[saleAttributionStatusIndex])
    }));
}
...
export function buildRopDailyControlWorkbook({
  ...
  for (const payment of paymentRows(paymentValues)) {
    ...
    const creditedManager = payment.saleAttributionStatus === 'OK_SALE_EMPLOYEE'
      ...
      : '';
    const attributionStatus = payment.saleAttributionStatus === 'OK_SALE_EMPLOYEE'
      ? (creditedManager ? 'OK_SALE_EMPLOYEE' : 'SALE_EMPLOYEE_NOT_IN_ACTIVE_PLAN')
      : (payment.saleAttributionStatus || 'SALE_ATTRIBUTION_INVALID');
  ...
  for (let day = 1; day <= maxDay; day += 1) {
    ...
    for (const plan of plans) {
      ...
      const noteParts = [];
      if (!singleManager) noteParts.push('Общий филиальный план; личный факт считается по сотруднику, проводившему продажу в АШК.');
