...
import {
  ...
  isPaymentActionableObligationStatus,
...
export function buildDecisionFinancialSnapshot({
  ...
  const unconfirmed = obligationRows.filter((row) => isUnconfirmedObligationStatus(row?.status));
  const unconfirmedAmounts = unconfirmed.map((row) => {
    const amount = finiteOrNull(row?.remaining);
    return amount !== null && amount > 0 ? amount : null;
  });

  const criticalPayments = obligationRows
    .filter((row) => isPaymentActionableObligationStatus(row?.status))
    .map((row) => {
      const id = String(row?.id || '').trim();
      const unconfirmedStatus = isUnconfirmedObligationStatus(row?.status);
      const remaining = finiteOrNull(row?.remaining);
      if (remaining === null && !unconfirmedStatus) {
        throw new Error(`open confirmed obligation amount is missing: ${id || '<unknown>'}`);
      }
      const amount = remaining === null || (unconfirmedStatus && remaining <= 0)
        ? null
    ...
    .filter((row) => row.id && row.dueDate && (row.amount === null || row.amount > 0));
  ...
  return {
    ...
    receivables: {
      contracts: Math.max(Math.trunc(finiteOrNull(receivables?.contracts) ?? 0), 0),
      debt: Math.max(finiteOrNull(receivables?.debt) ?? 0, 0),
      sales: Math.max(finiteOrNull(receivables?.sales) ?? 0, 0),
      paid: Math.max(finiteOrNull(receivables?.paid) ?? 0, 0)
    },
    obligations: {
      ...
      unconfirmed: {
        ...
        amount: unconfirmed.length === 0
          ? 0
          : unconfirmedAmounts.every((value) => value !== null)
            ? unconfirmedAmounts.reduce((sum, value) => sum + value, 0)
            : null,
