  ...
  try {
    ...
    if (!response?.ok) {
      ...
      if (typeof response?.json === 'function') {
        try {
          errorType = gatewayPayloadReason(await response.json());
        } catch {
          errorType = '';
        }
      }
      return { ok: false, status, errorType };
    }
    try {
...
export async function probeCashPhotoGateway({
  ...
  if (!response.ok) {
    const suffix = response.errorType ? `_${response.errorType}` : '';
    return { ok: false, reason: response.reason || `HTTP_${response.status}${suffix}`, models: [] };
  }
...
export async function recognizeCashPhotoViaGateway({
  ...
  if (!catalog.ok) {
    const suffix = catalog.errorType ? ` ${catalog.errorType}` : '';
    throw retryableError([`gateway catalog: ${catalog.reason || `HTTP ${catalog.status}${suffix}`}`]);
  }
  ...
  for (const model of models) {
    ...
    const suffix = response.errorType ? ` ${response.errorType}` : '';
    const detail = response.reason || `HTTP ${response.status}${suffix}`;
    diagnostics.push(`${model}: ${detail}`);
    if (response.status === 403 && response.errorType === 'customer_verification_required') {
