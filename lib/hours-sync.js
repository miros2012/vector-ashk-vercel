...
function normalizedRow(row, month, loadedAt) {
  const rawFactStart = text(row?.FactStart);
  const date = businessDateFromFactStart(rawFactStart);
  const businessStart = normalizedBusinessStart(rawFactStart);
  if (!date || !businessStart) {
    throw new Error(`ASHK row has invalid FactStart: ${rawFactStart || '(missing FactStart)'}`);
  }
  const values = {
    month,
    factDate: date,
    factStart: rawFactStart,
...
function toReconciliationValues(metrics, month, loadedAt, sourceRows, duplicateRows, outOfMonthRows) {
  const rows = [
    ...
    ['control', 'Out-of-month source rows ignored', outOfMonthRows, '', month, loadedAt],
...
export function buildHoursImportWorkbook(apiRows, { month, loadedAt }) {
  ...
  let inMonthSourceRows = 0;

  for (const apiRow of apiRows) {
    const row = normalizedRow(apiRow, month, loadedAt);
    if (!row.factDate.startsWith(`${month}-`)) {
      outOfMonthRows += 1;
      continue;
    }
    inMonthSourceRows += 1;
  ...
  const duplicateRows = inMonthSourceRows - rows.length;
  return {
    ...
    reconciliationValues: toReconciliationValues(
      metrics,
      month,
      loadedAt,
      apiRows.length,
