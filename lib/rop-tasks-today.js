...
function selectPriorityDebtors(allDebtors, minimumRows, collectionTarget) {
  const selected = allDebtors.slice(0, minimumRows);
  const target = Math.max(0, roundMoney(toNumber(collectionTarget)));
  let selectedDebt = roundMoney(selected.reduce((sum, item) => sum + item.debt, 0));
  for (let index = selected.length; index < allDebtors.length && selectedDebt + 0.01 < target; index += 1) {
    selected.push(allDebtors[index]);
    selectedDebt = roundMoney(selectedDebt + allDebtors[index].debt);
  }
  return selected;
}
...
export function buildRopDebtorPriority({ receivablesValues, taskValues, planValues, limitPerBranch = 5 } = {}) {
  ...
  const collectionTargetIndex = taskIdx.get('План сбора ДЗ, ₽');
  ...
  const problems = (Array.isArray(taskValues) ? taskValues.slice(1) : [])
    ...
    .map(row => ({
      ...
      collectionTarget: collectionTargetIndex === undefined
        ? 0
        : roundMoney(toNumber(row[collectionTargetIndex])),
  ...
  for (const problem of problems) {
    ...
    const priorityDebtors = selectPriorityDebtors(allDebtors, maxRows, problem.collectionTarget);
    for (const item of priorityDebtors) {
