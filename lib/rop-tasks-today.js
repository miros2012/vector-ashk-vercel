...
const HEADERS = [
  ...
  'План к дате','Факт вчера','Факт с начала месяца','Дефицит, ₽','Прогноз месяца',
  'Текущая ДЗ','Задача','Срок','Статус исполнения','Примечание'
];

const REQUIRED = [
  'Дата отчёта','Уровень','Менеджер','Филиал','План месяца','План к дате',
  'Факт вчера','Факт с начала месяца','Дефицит к плану на дату','Прогноз месяца',
...
function assertConsistent(existing, row, idx) {
  const fields = [
    'План месяца','План к дате','Факт вчера','Факт с начала месяца',
...
export function buildRopTasksToday({ morningValues, taskDate } = {}) {
  ...
  const rows = (Array.isArray(morningValues) ? morningValues.slice(1) : [])
  ...
  for (const [branch, group] of byBranch.entries()) {
    ...
    output.push([
      ...
      roundMoney(toNumber(row[idx.get('Факт вчера')])),
