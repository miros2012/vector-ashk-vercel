...
const DEBTOR_HEADERS = [
  '№','Приоритет филиала','Филиал','Дефицит филиала','ДЗ филиала','StudentId','Договор',
  'Менеджер АШК','Ответственный','Долг','Последняя оплата','Причина приоритета','Действие','Срок'
];
...
export function buildRopDebtorPriority({ receivablesValues, taskValues, planValues, limitPerBranch = 5 } = {}) {
  const receivableIdx = headerMap(receivablesValues, [
    'StudentId','Филиал','Менеджер','Договор','Долг','Последняя оплата'
  ], 'Receivables detail');
  ...
  for (const problem of problems) {
    ...
    for (const item of allDebtors.slice(0, maxRows)) {
      ...
      output.push([
        ...
        text(item.row[receivableIdx.get('Договор')]),
        owner,
        responsible,
        item.debt,
        lastPayment,
        reasons.join('; '),
        'Связаться сегодня; подтвердить сумму и дату оплаты; зафиксировать обещание.',
        problem.deadline
      ]);
    }
  }
  ...
  return {
    ...
    metrics: {
      ...
      prioritizedDebt: roundMoney(output.reduce((sum, row) => sum + toNumber(row[9]), 0))
    }
  };
}
