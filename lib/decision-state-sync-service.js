...
export function createDecisionStateSynchronizer({
  ...
  return async function synchronizeDecisionState({ dryRun = true } = {}) {
    ...
    // Drift is the condition commit-mode reconciliation is designed to repair.
    // Safety comes from a formula backup before the atomic H/J/M/P write and
    // mandatory post-write shadow verification with rollback on any failure.
