...
function asArray(payload) {
  ...
  throw Object.assign(new Error('ASHK response validation failed'), { errorClass: 'VALIDATION', statusCode: 422 });
}
...
export function createAshkReceivablesSource({
  ...
  async function getJson(path, publicError) {
    ...
    try {
      ...
      if (!response.ok) throw new Error('ashk_error');
      ...
      throw Object.assign(new Error(publicError), { errorClass, statusCode });
    }
  }

  async function fetchStudent(studentIdRaw) {
    const studentId = Number(studentIdRaw);
    if (!Number.isInteger(studentId) || studentId <= 0) {
      throw new Error('StudentId must be a positive integer');
    }
    const payload = await getJson(
      `/api/StudentExternalGet?param=${encodeURIComponent(studentId)}`,
      'ASHK StudentExternalGet failed'
    );
    const student = payload?.data ?? payload;
    if (!student || typeof student !== 'object' || Array.isArray(student)) {
      throw new Error('ASHK StudentExternalGet failed');
    }
    return student;
  }
  ...
  return { fetchCurrent, fetchStudent };
}
