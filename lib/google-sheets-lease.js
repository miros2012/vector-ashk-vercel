...
export async function boundedGoogleSheetsRequest(execute, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS, phase = 'unknown') {
  if (typeof execute !== 'function') throw new Error('Google Sheets request is required');
  const timeout = Number(timeoutMs);
  if (!Number.isFinite(timeout) || timeout <= 0) {
    throw new Error('requestTimeoutMs is invalid');
  }

  let timer;
  try {
    return await Promise.race([
      Promise.resolve().then(() => execute({ timeout, retry: false })),
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          const error = new Error('Google Sheets request timed out');
          error.phase = phase;
          reject(error);
        }, timeout);
      })
...
async function readRows({ sheets, spreadsheetId, escapedTitle, requestTimeoutMs }) {
  const response = await boundedGoogleSheetsRequest(options => sheets.spreadsheets.values.get({
    spreadsheetId,
    range: `'${escapedTitle}'!A:B`,
    valueRenderOption: 'UNFORMATTED_VALUE'
  }, options), requestTimeoutMs, 'lease-read');
  return response?.data?.values || [];
}

async function findSheetId({ sheets, spreadsheetId, title, requestTimeoutMs }) {
  const response = await boundedGoogleSheetsRequest(options => sheets.spreadsheets.get({
    spreadsheetId,
    fields: 'sheets.properties(sheetId,title)'
  }, options), requestTimeoutMs, 'lease-metadata');
...
async function compareAndSwap({
  ...
  const response = await boundedGoogleSheetsRequest(options => sheets.spreadsheets.batchUpdate({
    spreadsheetId,
    requestBody: {
      requests: [{
        findReplace: {
  ...
  }, options), requestTimeoutMs, 'lease-cas');
...
export async function withGoogleSheetsLease({
  ...
  const rows = await readRows({ sheets, spreadsheetId: id, escapedTitle, requestTimeoutMs });
  ...
  const sheetId = await findSheetId({ sheets, spreadsheetId: id, title, requestTimeoutMs });
