...
export async function boundedGoogleSheetsRequest(execute, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS) {
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error('Google Sheets request timed out')), timeout);
      })
    ]);
  } finally {
    clearTimeout(timer);
