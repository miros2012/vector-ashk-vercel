...
export function reconcileMasterPayroll({ gross, evidence, requiredBlockedTypes = [] }) {
  ...
  const coreGatesOk = coreGateNames.every((name) => gates[name]);
  const reviewGatesOk = reviewGateNames.every((name) => gates[name]);
  const promotionStatus = !coreGatesOk
    ? 'BLOCKED'
    : reviewGatesOk
      ? 'READY'
      : 'VERIFIED_WITH_OPEN_REVIEW';
  ...
  const finalMasters = masters.map((master) => ({
    ...
    status: master.status === 'REVIEW_REQUIRED'
      ...
      : promotionStatus === 'READY'
        ? 'READY'
        : promotionStatus === 'VERIFIED_WITH_OPEN_REVIEW'
          ? 'VERIFIED'
          : 'INTERIM'
  }));
  ...
  return {
    ...
    gates,
    gateClasses: {
      core: Object.fromEntries(coreGateNames.map((name) => [name, gates[name]])),
      openReview: Object.fromEntries(reviewGateNames.map((name) => [name, gates[name]]))
    },
    promotionStatus
  };
}
