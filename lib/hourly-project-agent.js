...
const RETRYABLE_MODEL_DENIALS = new Set([
...
function safeGatewayIdentifier(value) {
  const candidate = text(value).toLowerCase();
  return /^[a-z][a-z0-9_]{0,79}$/.test(candidate) ? candidate : '';
}

function knownGatewayMessageClass(value) {
  const message = text(value).toLowerCase();
  if (!message) return '';
  if (/restricted access to (?:this )?model|model (?:is )?(?:disabled|not allowed)/.test(message)) {
    return 'model_restricted';
  }
  if (/invalid (?:authentication|api key|token)|authentication (?:failed|required)|unauthorized|forbidden/.test(message)) {
    return 'authentication_denied';
  }
  return '';
}

function gatewayPayloadReason(payload) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return '';
  const nested = payload.error && typeof payload.error === 'object' && !Array.isArray(payload.error)
    ? payload.error
    : {};
  for (const candidate of [payload.type, payload.code, nested.code, nested.type]) {
    const identifier = safeGatewayIdentifier(candidate);
    if (identifier) return identifier;
  }
  const message = typeof payload.error === 'string'
    ? payload.error
    : nested.message ?? payload.message;
  return knownGatewayMessageClass(message);
}

async function gatewayHttpError(stage, response) {
  const status = Number(response?.status || 0);
  let reason = '';
  if (typeof response?.json === 'function') {
    try {
      reason = gatewayPayloadReason(await response.json());
    } catch {
      reason = '';
    }
  }
  const suffix = reason ? ` type ${reason}` : '';
  const error = new Error(`gateway ${stage} failed with status ${status}${suffix}`);
  error.gatewayFailure = Object.freeze({ stage, status, reason });
  return error;
}

function generationFailureCode(error) {
  const message = String(error?.message || '').trim();
  const gatewayHttp = message.match(/^gateway (catalog|completion) failed with status (\d{3})(?: type ([a-z][a-z0-9_]{0,79}))?$/);
  if (gatewayHttp) {
    const suffix = gatewayHttp[3] ? `_${gatewayHttp[3]}` : '';
    return `gateway_${gatewayHttp[1]}_http_${gatewayHttp[2]}${suffix}`;
  }
  if (message === 'gateway token is required' || message === 'gateway token is too long') {
    return 'gateway_token_invalid';
  }
  if (message === 'no supported AI Gateway model is available') return 'gateway_model_unavailable';
  if (message === 'gateway response is invalid') return 'gateway_response_invalid';
  if (message === 'gateway returned invalid structured output') return 'gateway_output_invalid_json';
  if (message === 'gateway completion content is missing') return 'gateway_output_missing_content';
  if (message === 'gateway probe marker mismatch') return 'gateway_probe_marker_mismatch';
  if (/^(agent proposal|agent change|change reason|proposal summary|reviewReason)/i.test(message)) {
    return 'gateway_proposal_invalid';
  }
  return 'gateway_unknown';
}

function isRetryableModelDenial(error) {
  const failure = error?.gatewayFailure;
  return failure?.stage === 'completion'
    && failure?.status === 403
    && RETRYABLE_MODEL_DENIALS.has(failure?.reason);
}
...
export function chooseGatewayModels(models = []) {
  const available = new Set((Array.isArray(models) ? models : [])
    .map((model) => text(typeof model === 'string' ? model : model?.id))
    .filter(Boolean));
  const candidates = SUPPORTED_MODELS.filter((model) => available.has(model));
  if (!candidates.length) throw new Error('no supported AI Gateway model is available');
  return candidates;
}

export function chooseGatewayModel(models = []) {
  return chooseGatewayModels(models)[0];
}
...
async function callGateway({ fetchImpl, gatewayToken, model, messages, maxTokens = 12_000 }) {
  ...
  if (!response?.ok) throw await gatewayHttpError('completion', response);
...
async function callGatewayWithFallback({
  ...
  for (let index = 0; index < models.length; index += 1) {
    ...
    try {
      const result = await callGateway({ fetchImpl, gatewayToken, model, messages, maxTokens });
      return { ...result, model };
    } catch (error) {
      lastError = error;
      const hasNext = index + 1 < models.length;
      if (!hasNext || !isRetryableModelDenial(error)) throw error;
    }
  }
  throw lastError || new Error('no supported AI Gateway model is available');
}
...
export function createHourlyProjectAgentService({
  ...
  return async function hourlyProjectAgentService({ authorization, body } = {}) {
    ...
    try {
      ...
      if (!modelResponse?.ok) throw await gatewayHttpError('catalog', modelResponse);
      const modelPayload = await responseJson(modelResponse);
      const models = chooseGatewayModels(modelPayload?.data || modelPayload?.models || []);

      if (mode === 'hourly_agent_probe') {
        const { parsed, model } = await callGatewayWithFallback({
          fetchImpl,
          gatewayToken,
          models,
          messages: [
      ...
      const { parsed, usage, model } = await callGatewayWithFallback({
        fetchImpl,
        gatewayToken,
        models,
        messages: [
      ...
      logger.error?.('hourly-agent-generation:', generationFailureCode(error));
