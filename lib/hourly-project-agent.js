...
function normalizePath(value) {
  ...
  if (!path || path.startsWith('/') || path.includes('\0') || path.split('/').includes('..')) {
    throw new Error(`unsafe file path: ${path || '(empty)'}`);
  }
  if (!SAFE_ROOTS.some((root) => path.startsWith(root))) {
    throw new Error(`file path is outside allowed roots: ${path}`);
  }
  if (path.startsWith('api/') || path.startsWith('scripts/') || path.startsWith('.github/')) {
    throw new Error(`file path is not allowed: ${path}`);
  }
  if (/^docs\/(?:.*\/)?(?:secrets?|credentials?)(?:\.|\/|$)/i.test(path)) {
    throw new Error(`file path is not allowed: ${path}`);
  }
  return path;
}
...
export function authorizeHourlyAgentClaims(claims = {}) {
  const expectedSubject = `repo:${EXPECTED.repository}:ref:${EXPECTED.ref}`;
  const suppliedSubject = text(claims.sub);
  const subjectMatches = !suppliedSubject || suppliedSubject === expectedSubject;
  ...
  if (eventName === 'issues' && String(claims.actor_id) !== EXPECTED.ownerId) {
...
export function createHourlyProjectAgentService({
  ...
  return async function hourlyProjectAgentService({ authorization, body } = {}) {
    ...
    try {
      ...
      if (claims?.repository || claims?.workflow_ref) authorizeHourlyAgentClaims(claims);
    } catch (error) {
    ...
    try {
      ...
      requiredText(body?.requestId, 'requestId', MAX_REQUEST_ID);
    } catch (error) {
