...
export function createCashPhotoStore({
  ...
  async function markDuplicateVerified(photo) {
    const branch = String(photo?.branch || '').trim();
    ...
    const expectedWallet = `Касса ${branch}`;
    const index = rows.findIndex(row => String(row?.[1] || '').trim() === expectedWallet);
    if (index < 0) return 'skipped-missing';
    const active = rows[index]?.[3];
    if (active === false || String(active).trim().toUpperCase() === 'FALSE') return 'skipped-inactive';
    const row = index + 2;
    await updateRange(`'${CASH_WALLETS_SHEET}'!J${row}`, [now().toISOString()]);
    return 'verified';
  }
  ...
  return {
    ...
    async markDuplicateVerified(photo) {
      return markDuplicateVerified(photo);
    },
    ...
    async listPending(limit = 5, filters = {}) {
      ...
      for (let index = 0; index < rows.length && pending.length < limit; index += 1) {
        ...
        const staleRecognizing = status === 'Распознавание'
          && (!Number.isFinite(startedMs) || currentMs - startedMs >= RECOGNIZING_STALE_MS);
    ...
    async reconcileDraftBacklog(limit = 100) {
      if (!journalPipeline?.reconcileDraftBacklog) {
        return {
          scanned: 0,
          promoted: 0,
          recovered: 0,
      ...
      return journalPipeline.reconcileDraftBacklog(limit);
    },
