...
export function evaluateTochkaDdsCoverage({
  ...
  const manualClassificationKeys = new Set(
    (Array.isArray(controlRows) ? controlRows : [])
      .filter(row => text(row?.[15]) === 'Ручная классификация')
      .map(row => text(row?.[10]))
      .filter(Boolean)
  );
  ...
  const eligible = uniqueByKey((Array.isArray(tochkaRows) ? tochkaRows : []).filter((row) => {
    ...
    const key = text(row?.[14]);
    return date !== null
      && Math.trunc(date) === Math.trunc(targetDate)
      && !isInternal
      && Boolean(key)
      && !manualClassificationKeys.has(key);
  }));
  ...
  return {
    ...
    missingOutflow,
    missingInflow,
    missingFingerprint: createHash('sha256').update(JSON.stringify(missing.map(row=>text(row?.[14])).sort())).digest('hex'),
