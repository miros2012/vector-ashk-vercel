...
export function createDecisionShadowSheetAdapter({ sheets, spreadsheetId, now = () => new Date() }) {
  ...
  return {
    async run() {
      ...
      const catalog = parseCatalog(ranges[3]?.values || []);
      const currentDecisions = parseDecisions(ranges[4]?.values || []);
      const snapshot = buildDecisionFinancialSnapshot({ ...forecast, adjustmentRows, obligationRows });
