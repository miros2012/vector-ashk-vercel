...
export function createOwnerDashboardApi({secret,readPackage,now=Date.now,googleOnly=false}) {
  return async (req,res) => {
    ...
    if(session) {
      if(googleOnly)return res.status(403).json({ok:false,error:'google_sign_in_required'});
      let body=req.body;
