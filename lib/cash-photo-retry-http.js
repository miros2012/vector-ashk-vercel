...
const GOOGLE_OAUTH_REASONS = new Set([
  'OK',
  'API_DISABLED',
  ...
  return async function cashPhotoRetryHttpHandler(req, res) {
    ...
    if (method === 'GET' && requestParam(req, 'diagnostic') === 'GOOGLE_OAUTH') {
      try {
        ...
        const candidateReason = String(result?.reason || 'UPSTREAM_ERROR');
        const reason = GOOGLE_OAUTH_REASONS.has(candidateReason) ? candidateReason : 'UPSTREAM_ERROR';
