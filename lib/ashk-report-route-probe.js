...
export function extractAshkReportRouteCandidates(html) {
  ...
  const relevance = /report|finance|payment|employee|activity|menu|nav|analytics?|statistics?|staff|personnel|отч[её]т|финанс|оплат|сотруд|активност|меню|аналит|статист/i;
  let match;
...
export function extractAshkRouteStrings(source) {
  const routes = [];
  const relevance = /report|finance|payment|employee|activity|menu|navigation|nav|analytics?|statistics?|staff|personnel/i;
  const quoted = /["']([^"'\r\n]+)["']/g;
  let match;
...
export async function probeAshkReportRoutes({ session, maxAssets = 24 } = {}) {
