const EXPECTED = Object.freeze({
  ...
  ownerId: '46207692',
  ref: 'refs/heads/main',
  workflowRefs: Object.freeze([
    'miros2012/vector-ashk-vercel/.github/workflows/hourly-project-continuation.yml@refs/heads/main',
    'miros2012/vector-ashk-vercel/.github/workflows/finance-recovery.yml@refs/heads/main'
  ]),
...
export function authorizeFinanceSyncClaims(claims = {}) {
  ...
  const valid = claims.iss === EXPECTED.issuer
    ...
    && EXPECTED.workflowRefs.includes(claims.workflow_ref)
...
export function createGitHubFinanceSyncHandler({
  ...
  if (typeof runRecovery !== 'function') throw new Error('runRecovery is required');
  ...
  return async function githubFinanceSyncHandler(req, res) {
    ...
    if (body.mode !== 'finance_sync' || !['intraday', 'recovery', 'full'].includes(body.kind)) {
    ...
    const runner = body.kind === 'full'
      ? runFull
      : body.kind === 'recovery' ? runRecovery : runIntraday;
    ...
    const childReq = {
      ...
      headers: {
        ...
        ...(body.kind === 'recovery' ? { 'x-vector-finance-recovery-only': 'true' } : {})
      },
      query: {},
      body: {}
    };
