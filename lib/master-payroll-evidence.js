...
function confirmedItem({ masterKey, type, amount, sourceId, row }) {
  const item = { masterKey, type, amount, sourceId, status: 'CONFIRMED' };
  if (row.settlementGroup) item.settlementGroup = String(row.settlementGroup);
  return item;
}
...
export function normalizePayrollEvidence(rows, aliases = {}) {
  ...
  for (const row of rows) {
    ...
    confirmed.push(confirmedItem({ masterKey, type, amount, sourceId, row }));
  }
