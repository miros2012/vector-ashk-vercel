...
export function createCashPhotoUploadService({ store, recognize } = {}) {
  ...
  async function prepare(input = {}) {
    ...
    if (existing && String(existing.branch || '').trim() !== normalized.branch) {
      return { terminal: { statusCode: 409, body: {
    ...
    if (existing && isRecognizedStatus(existing.status)) {
      try {
        await store.markDuplicateVerified?.(existing);
      } catch (error) {
        console.error('cash photo duplicate freshness refresh failed', error?.name || 'Error');
      }
      return { terminal: {
        statusCode: 200,
        body: {
          ...
          message: 'Это фото уже было принято и распознано. Повторная отправка подтверждает, что журнал не изменился.'
        }
      } };
    }

    const photo = existing || await store.persistPhoto({ ...normalized, hash });
    return { normalized, photo, existing: Boolean(existing) };
  }
  ...
  return {
    async save(input = {}) {
      ...
      await store.markPending?.(photo, {
        message: 'Фото сохранено. Распознавание продолжится автоматически.',
        diagnostics: []
      });
      return {
        statusCode: 202,
    ...
    async upload(input = {}) {
      const prepared = await prepare(input);
      if (prepared.terminal) return prepared.terminal;
      const { normalized, photo } = prepared;
