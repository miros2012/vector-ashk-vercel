...
export function createCashPhotoUploadHttpHandler({ authorize, uploadService } = {}) {
  ...
  if (!uploadService?.save && !uploadService?.upload) throw new Error('Cash photo upload service is required');
  ...
  return async function cashPhotoUploadHttpHandler(req, res) {
    ...
    try {
      const persist = uploadService.save || uploadService.upload.bind(uploadService);
