...
export function calculateDecisionEffectiveness({ decisions = [], history = [] } = {}) {
  const syntheticRuleIds = new Set(decisions
    .filter((decision) => decision?.synthetic)
    .map((decision) => String(decision?.ruleId || '').trim())
    .filter(Boolean));
  const normalizedDecisions = decisions.filter((decision) =>
    !decision?.synthetic && String(decision?.ruleId || '').trim()
  );
  const uniqueHistory = dedupeHistory(history)
    .filter((event) => !syntheticRuleIds.has(event.ruleId));
