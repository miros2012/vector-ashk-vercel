...
export function createCashPhotoAccessStore({
  ...
  sharedTokenSha256 = CASH_PHOTO_SHARED_TOKEN_SHA256,
  legacySharedTokenSha256 = process.env.CASH_PHOTO_SHARED_TOKEN_SHA256,
  allowBareShared = false
} = {}) {
  ...
  const sharedHashes = [...new Set([sharedTokenSha256, legacySharedTokenSha256]
  ...
  return {
    ...
    async authorize(token) {
      ...
      if (!token && allowBareShared && branches.length) {
        return { accessId: 'STAFF:BARE', role: 'Сотрудники', branches: branches.map(branch => ({ ...branch })) };
      }
      if (!/^[A-Za-z0-9_-]{43}$/.test(token)) return null;
      const hash = createHash('sha256').update(token).digest();
      if (branches.length && sharedHashes.some(sharedHash => timingSafeEqual(hash, sharedHash))) {
