...
const missing='Нет подтверждённых данных';
...
function showLogin(){clearData();$('login').hidden=false;$('logout').hidden=true;}
async function api(path,options={}){const response=await fetch('/api/owner-dashboard-'+path,{credentials:'same-origin',cache:'no-store',...options});let body;try{body=await response.json();}catch{body={};}if(!response.ok){const error=new Error(body.error || 'network_error');error.status=response.status;throw error;}return body;}
function rows(id,entries){const root=$(id);root.replaceChildren();for(const [label,value,isCount=false] of entries){const row=node('div');const dd=node('dd',isCount ? value===null?missing:String(value) : money(value),value===null?'missing':value<0?'negative':'');row.append(node('dt',label),dd);root.append(row);}}
function renderForecast(){const root=$('forecast');root.replaceChildren();const names={base:'Базовый',conservative:'Консервативный',target:'Целевой'};const start=Date.parse(dashboard.businessDate);const end=start+days*86400000;let any=false;for(const scenario of dashboard.scenarios){const eligible=scenario.daily.filter(d=>Date.parse(d.date)>=start&&Date.parse(d.date)<=end).sort((a,b)=>a.date.localeCompare(b.date));const point=eligible.at(-1);any ||= !!point;const tr=node('tr');tr.append(node('td',names[scenario.name]),node('td',money(point?.closingBalance),point?.closingBalance<0?'negative':''),node('td',point?date(point.date):'Нет данных'),node('td',money(scenario.minimumBalance)+(scenario.minimumBalanceDate?' · '+date(scenario.minimumBalanceDate):''),scenario.minimumBalance<0?'negative':''));root.append(tr);}if(!dashboard.scenarios.length){const td=node('td',missing);td.colSpan=4;const tr=node('tr');tr.append(td);root.append(tr);}$('forecast-note').textContent=any?'Остаток показан на последнюю доступную дату в выбранном периоде.':'На выбранный период нет подтверждённого прогноза.';}
function render(data){dashboard=data;const m=data.metrics;$('login').hidden=true;$('dashboard').hidden=false;$('logout').hidden=false;const generated=new Date(data.generatedAt);const time=data.generatedAt&&Number.isFinite(generated.getTime())?new Intl.DateTimeFormat('ru-RU',{hour:'2-digit',minute:'2-digit',timeZone:'Asia/Yekaterinburg'}).format(generated):'—';$('updated').textContent=`Отчётная дата: ${date(data.businessDate)} · Получено: ${date(data.generatedAt)}, ${time} (Екатеринбург)`;
 const banner=$('health-banner');banner.className='banner '+(data.health.status==='BLOCKED'?'blocked':data.health.status==='WARNING'?'warning':'');banner.textContent=healthNames[data.health.status]+(data.blockers.length?' · Расчёт вывода средств ограничен.':'');
 const cards=[['Доступные деньги',m.availableCash,'Подтверждённый остаток из источника'],['Безопасно к выводу',m.safeWithdrawal,data.withdrawalBlocked?'Вывод ограничен: проверьте причины ниже':'С учётом резервов и ограничений системы'],['Кассовый разрыв',m.cashGap,'Базовый сценарий · весь горизонт источника']];$('money').replaceChildren();for(const [label,value,note] of cards){const card=node('article',undefined,'card');card.append(node('div',label,'card-label'),node('div',money(value),'amount'+(value===null?' missing':'')),node('div',note,'card-note'));$('money').append(card);}
